...
export default function ReceiptPage() {
  const paymentId = useRouteId("paymentId", "receipt");
  if (!paymentId) return null;
  return <ReceiptView paymentId={paymentId} />;
}

function ReceiptView({ paymentId }: { paymentId: string }) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-4 flex justify-end gap-2 print:hidden">
        ...
        <Button onClick={() => window.print()}><Icon name="printer" className="h-4 w-4" /> Print receipt</Button>
      </div>
            ...
            <tbody>
              {invoice.items.map((it) => (
                <tr key={it.id} className="border-b border-slate-100">
                  ...
                  <td className="py-1.5 text-right tabular-nums text-slate-700">{formatMoney(it.amount, invoice.currency)}</td>
                </tr>
              ))}
              <tr>
                <td className="py-2 text-sm font-semibold text-slate-900">Amount paid</td>
