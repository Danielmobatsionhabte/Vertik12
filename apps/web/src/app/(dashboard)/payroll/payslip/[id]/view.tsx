...
export default function PayslipPage() {
  const id = useRouteId("id", "payslip");
  if (!id) return null;
  return <PayslipView id={id} />;
}

function PayslipView({ id }: { id: string }) {
      ...
      <div className="mb-4 flex flex-wrap justify-end gap-2 print:hidden">
        ...
        {canEmail && (
          <Button variant="secondary" loading={emailing} onClick={() => void emailPaystub()}>
            <Icon name="mail" className="h-4 w-4" /> Email to {staff.user.firstName}
          </Button>
        )}
        <Button onClick={() => window.print()}><Icon name="printer" className="h-4 w-4" /> Print paystub</Button>
      </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              <tr className="border-b border-slate-100">
                <td className="py-1.5 text-slate-700">Basic salary</td>
                <td className="py-1.5 text-right tabular-nums">{formatMoney(data.basicSalary, data.currency)}</td>
              </tr>
              {data.allowances.map((a) => (
                <tr key={a.name} className="border-b border-slate-100">
                  <td className="py-1.5 text-slate-700">{a.name} <span className="text-xs text-slate-400">(allowance)</span></td>
                  <td className="py-1.5 text-right tabular-nums text-emerald-700">+{formatMoney(a.amount, data.currency)}</td>
                </tr>
              ))}
              {data.bonus > 0 && (
                <tr className="border-b border-slate-100">
                  <td className="py-1.5 text-slate-700">Bonus</td>
                  <td className="py-1.5 text-right tabular-nums text-emerald-700">+{formatMoney(data.bonus, data.currency)}</td>
                </tr>
              )}
              {data.deductions.map((d) => (
                <tr key={d.name} className="border-b border-slate-100">
                  <td className="py-1.5 text-slate-700">{d.name} <span className="text-xs text-slate-400">(deduction)</span></td>
                  <td className="py-1.5 text-right tabular-nums text-rose-600">−{formatMoney(d.amount, data.currency)}</td>
                </tr>
              ))}
              <tr className="border-b border-slate-100">
                <td className="py-1.5 text-slate-500">Gross pay</td>
                <td className="py-1.5 text-right tabular-nums text-slate-700">{formatMoney(data.gross, data.currency)}</td>
              </tr>
              <tr>
                <td className="py-2 text-sm font-semibold text-slate-900">Net pay</td>
