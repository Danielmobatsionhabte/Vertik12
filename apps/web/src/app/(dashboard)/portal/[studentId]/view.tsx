...
export default function ChildPage() {
  const studentId = useRouteId("studentId", "portal");
  if (!studentId) return null;
  return <ChildView studentId={studentId} />;
}

function ChildView({ studentId }: { studentId: string }) {
              ...
              {homework.slice((homeworkPage - 1) * PER_PAGE, homeworkPage * PER_PAGE).map((a) => (
                <li key={a.id} className="px-6 py-4">
                  ...
                  {a.attachmentName && (
                    ...
                    >
                      <Icon name="paperclip" className="h-3.5 w-3.5" /> {a.attachmentName} (from the teacher)
                    </button>
                  )}
          ...
          <div className="space-y-2">
            ...
            {attachment && (
              <p className="flex items-center gap-2 text-xs text-slate-600">
                <Icon name="paperclip" className="h-3.5 w-3.5" /> {attachment.name}
