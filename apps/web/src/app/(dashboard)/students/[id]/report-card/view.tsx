...
export default function ReportCardPage() {
  ...
  if (!id) return null;
  return <ReportCardView id={id} />;
}

function ReportCardView({ id }: { id: string }) {
              ...
              <tbody>
                {card.subjects.map((s) => (
                  <tr key={s.subject.id} className="border-b border-slate-100">
                    <td className="py-1.5 pr-4 font-medium text-slate-800">{s.subject.name}</td>
                    <td className="py-1.5 pr-4 text-xs text-slate-500">
                      {s.exams.map((e) => `${e.exam}: ${e.marks}/${e.maxMarks}`).join(" · ")}
                    </td>
                    <td className="py-1.5 pr-4 text-right tabular-nums">{s.percentage}</td>
                    <td className="py-1.5 text-right font-semibold">{s.grade}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-slate-300 font-semibold text-slate-900">
                  ...
                  <td />
                  <td className="py-2 pr-4 text-right tabular-nums">{card.overall ?? "—"}</td>
                  <td className="py-2 text-right">{card.overallGrade ?? "—"} {card.gpa !== null && <span className="ml-2 text-xs font-normal text-slate-500">GPA {card.gpa}</span>}</td>
                </tr>
              </tfoot>
            </table>
          )}
