 ...
 * portal and submit on their child's behalf), then review submissions and
 * leave feedback/grades.
 */
...
export default function AssignmentsPage() {
  ...
  const load = useCallback(() => get<AssignmentRow[]>("/assignments").then(setRows), []);
  useEffect(() => {
    void load();
...
function CreateAssignmentModal({ open, mySubjects, onClose, onCreated }: {
  open: boolean;
  mySubjects: MySubject[];
  onClose: () => void;
  onCreated: () => Promise<void>;
}) {
  const [form, setForm] = useState({ classSubjectId: "", title: "", instructions: "", dueDate: "" });
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (open && mySubjects[0] && !form.classSubjectId) {
      setForm((f) => ({ ...f, classSubjectId: mySubjects[0]!.id }));
    }
  }, [open, mySubjects, form.classSubjectId]);
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setForm({ classSubjectId: mySubjects[0]?.id ?? "", title: "", instructions: "", dueDate: "" });
      await onCreated();
    } catch (err) {
  ...
  return (
    <Modal open={open} title="Send an assignment" onClose={onClose} wide>
        ...
        <div className="max-h-[60vh] space-y-4 overflow-y-auto pr-1">
          ...
          {detail.submissions.map((s) => (
            <div key={s.id} className="rounded-lg border border-slate-200 p-4">
              ...
              <p className="mt-2 whitespace-pre-wrap rounded bg-slate-50 p-3 text-sm text-slate-700">{s.content}</p>
              {s.linkUrl && (
                <a href={s.linkUrl} target="_blank" rel="noreferrer" className="mt-1 inline-block text-xs text-brand-600 underline">
                  Attached link
                </a>
              )}
              {s.feedback ? (
