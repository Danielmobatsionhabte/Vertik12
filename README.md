...
| Module | Capabilities |
| --- | --- |
| **Authentication & RBAC** | JWT access tokens + rotating refresh tokens, 7 roles (`SUPER_ADMIN`, `ADMIN`, `REGISTRAR`, `TEACHER`, `ACCOUNTANT`, `STUDENT`, `PARENT`), per-route role guards driven by a shared role→module matrix. **Administrator actions end sessions immediately** — a password reset, account deactivation or role change invalidates the access tokens already in the user's browser, so their portal signs out within seconds rather than when the token happens to expire |
...
| **Students** | Admissions with auto admission numbers (`VRT-2026-0001`), guardians, class enrolment, 360° profile (attendance %, finance summary, grades), soft withdrawal. Admin/Registrar **move a student between sections** (Grade 5 — A → Grade 5 — B) from the profile or straight off the list, with live seat counts; the move is refused if the section is full or belongs to another grade, and a student always holds exactly one enrolment per academic year |
| **Staff & HR** | Staff profiles with auto staff numbers, login accounts with generated temp passwords, off-boarding that disables logins. **Document files** — identification, background check, work authorization, qualifications, contracts — attached during registration or later from the staff member's file, with renewal dates and an **expiry chase-list** for what has lapsed or lapses soon. ADMIN-only to read as well as write (a background-check result is not payroll's business) |
...
| **School Calendar** 📅 | One shared calendar every portal reads — term dates, holidays, exam windows, meetings, sports days — audience-targeted (all / staff / students / parents). **Staff can add to it**: the administration publishes directly, other staff propose and an admin reviews. Families (parents & students) read only |
| **Timetable & Scheduling** 🗓️ | Registrar/admin build the weekly teaching schedule per academic year. Every placement is **conflict-checked** — a class can't be in two lessons at once, a teacher can't be in two rooms at once, and a room can't host two classes at once. Teachers see their own week and teaching load, and file a **change request** ("I can't make Tuesday 10:00") that the registrar approves — approving *moves the period for real*, re-running the same checks so a decision can never break the grid. Includes a "who is free then?" availability finder and **printable** class/teacher timetables on school letterhead |
...
GET|POST /staff/:id/documents         GET|PATCH|DELETE /staff/:id/documents/:docId
GET /staff/documents/expiring                                    (all ADMIN-only)
GET /dashboard/school       school letterhead for printouts (any signed-in role)
